...
export default function datarozhlasScrollyFullscreen(containerSelector, { onScrollDownToStep, onScrollUpFromStep }) {
  ...
  const stickyEl = containerEl.querySelector('.datarozhlas-scrolly-fullscreen-sticky');
  if (!stickyEl) {
    console.error(
      `Could not find sticky element with class .datarozhlas-scrolly-fullscreen-sticky under scrolly container specified by selector "${containerSelector}"`
    );
    return;
  }

  const firstStepEls = containerEl.querySelectorAll('.datarozhlas-scrolly-fullscreen-step');
  if (firstStepEls.length === 0) {
    console.error(
      `Could not find any step element with class .datarozhlas-scrolly-fullscreen-step under scrolly container specified by selector "${containerSelector}"`
    );
    return;
  }
    ...
    if (!scrollListenerTicking) {
      window.requestAnimationFrame(() => {
        ...
        if (containerTop > 0) {
          ...
          stickyEl.style.top = String(containerTop) + 'px';
  ...
  scroller
    .setup({
      // debug: true,
      offset: '100px',
      step: `${containerSelector} .datarozhlas-scrolly-fullscreen-step`,
    })
...
export const initDots = (scrollySelector) => {
  ...
  const stickyEl = scrollyEl.querySelector('.datarozhlas-scrolly-fullscreen-sticky');

  const dotsElement = document.createElement('div');
  dotsElement.classList.add('datarozhlas-scrolly-fullscreen-dots');
  stickyEl.append(dotsElement);

  scrollyEl.querySelectorAll('.datarozhlas-scrolly-fullscreen-step').forEach((stepElement, stepIndex) => {
